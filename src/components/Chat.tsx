...
import {
  ...
  perfectSelectionPrompt,
  rewriteShorterSelectionPrompt,
  en2cnPrompt,
  getTagsFromSelectionPrompt,
...
const Chat: React.FC<ChatProps> = ({
  ...
  const handleSaveAsNote = async (openNote = false) => {
    ...
    try {
      ...
      const now = new Date();
      const { fileName: timestampFileName, display, epoch } = formatDateTime(now);
...
${chatContent}`;
  ...
  useEffect(createEffect("extractKeySelection", extractKeyPrompt), []);
  useEffect(createEffect("simplifySelection", simplifyPrompt), []);
  useEffect(createEffect("ParagraphingSelection", ParagraphingPrompt), []);
  useEffect(createEffect("transalteSelectionToEnglish", cn2enPrompt), []);
  useEffect(
    createEffect("transalteSelectionToChinese", en2cnPrompt, { custom_temperature: 0.2 }),
    []
  );
  useEffect(
    createEffect("getTagsFromSelection", getTagsFromSelectionPrompt, {
  ...
  useEffect(createEffect("englishExplainSelection", englishExplainSelectionPrompt), []);
  useEffect(createEffect("perfectSelectionSelection", perfectSelectionPrompt), []);
  useEffect(
