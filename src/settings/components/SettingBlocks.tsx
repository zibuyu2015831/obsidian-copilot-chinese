    ...
    <div>
      ...
      <div className="add-custom-model">
        <h2 onClick={() => setIsAddModelOpen(!isAddModelOpen)} style={{ cursor: "pointer" }}>
          添加自定义模型 {isAddModelOpen ? "▼" : "▶"}
        </h2>
        {isAddModelOpen && (
          <div className="add-custom-model-form">
            <TextComponent
              name="模型名称"
              description={`例如 ${isEmbeddingModel ? "text-embedding-3-small" : "gpt-4o-mini"}`}
              value={newModel.name}
              placeholder="请输入模型名称"
              onChange={(value) => {
            ...
            <TextComponent
              name="Base URL (选填)"
              description="仅使用第三方代理时需要填入；其它情况请留空"
            ...
            <TextComponent
              name="API Key (选填)"
              description="填写第三方代理提供的API key；仅使用第三方代理时需要填入"
